...
// ---------------------------------------------------------------------------
// SDPALayer — Scaled-Dot-Product Attention on pre-projected Q, K^T, V.
//
// This is the lower half of an attention block: it takes Q / K^T / V tensors
// (the outputs of the QKV reshape+transpose chain inside the source graph)
// and replaces a (QK^T MatMul -> [Div/Mul scale] -> Softmax -> _·V MatMul ->
// Transpose([0,2,1,3]) -> Reshape) subgraph with a single MlasFlashAttention
// call. Used by graph_fusion_attention's SR-attention pass (Segformer, where
// Q comes from one input and K/V come from a spatially-reduced different
// input — so the projections can't be combined into one Attention layer).
class SDPALayerImpl CV_FINAL : public SDPALayer {
public:
    SDPALayerImpl(const LayerParams &params) {
        setParamsFrom(params);
        // `scale` is the divisor applied before softmax (i.e. sqrt(D)).
        // 0 / unset means "1" (no rescale).
    ...
    virtual bool getMemoryShapes(const std::vector<MatShape> &inputs,
        ...
        // Output mirrors the source graph's post-(Transpose+Reshape) tensor:
        // (B, H, S_q, D_v) -> Transpose([0,2,1,3]) -> (B, S_q, H, D_v) -> Reshape
        // -> (B, S_q, H*D_v). That's the contiguous layout MlasFlashAttention writes.
        outputs.assign(1, MatShape{B, S_q, H * D_v});

        // Internal scratch: K^T must be re-transposed to (B, H, S_kv, D) for
        // MlasFlashAttention. Allocated as a 1D buffer for simplicity.
                 ...
                 OutputArrayOfArrays internals_arr) CV_OVERRIDE
    {
        ...
        // Re-transpose K^T (B,H,D,S_kv) -> K (B,H,S_kv,D) into internals[0].
        // Tiny tensor (Segformer's worst case is ~64 KB), parallelised over
        // (b, h) for cache-friendly per-thread streaming.
        Mat &K = internals[0];
        const float* kT_data = KT.ptr<const float>();
        float*       k_data  = K.ptr<float>();
        const size_t kT_bhd_stride = (size_t)D * S_kv;          // (D, S_kv) tile
        const size_t k_bhd_stride  = (size_t)S_kv * D;          // (S_kv, D) tile
        cv::parallel_for_(cv::Range(0, B * H), [&](const cv::Range& r) {
            for (int bh = r.start; bh < r.end; bh++) {
                const float* src = kT_data + (size_t)bh * kT_bhd_stride;  // (D, S_kv)
                float*       dst = k_data  + (size_t)bh * k_bhd_stride;   // (S_kv, D)
        ...
        // Fallback: emulate the original unfused chain. Not perf-critical;
        // this only runs when MLAS isn't built in.
...
private:
    ...
    {
        // attn[b,h,i,j] = sum_d Q[b,h,i,d] * K[b,h,j,d];
        // attn /= scale; softmax over j; out[b,h,i,d'] = sum_j attn[i,j] * V[b,h,j,d'].
