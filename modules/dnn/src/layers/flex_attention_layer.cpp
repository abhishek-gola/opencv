...
// ONNX FlexAttention operator (ai.onnx.preview opset 1).
// Spec: scaled dot-product attention with GQA and optional score_mod / prob_mod
...
public:
    ...
    void forward(InputArrayOfArrays inputs_arr, OutputArrayOfArrays outputs_arr, OutputArrayOfArrays /*internals*/) CV_OVERRIDE
    {
        ...
        if (inputs[0].depth() == CV_64F)
            run<double>(inputs, outputs, false);
        else if (inputs[0].depth() == CV_16F)
            run<float>(inputs, outputs, true);
        else
            run<float>(inputs, outputs, false);
    }
