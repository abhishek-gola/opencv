...
// ONNX CausalConvWithState operator (ai.onnx opset 27).
// Depthwise (groups == channels) causal 1-D convolution over [past_state ; input] with an
// optional bias and optional silu/swish activation. Emits the convolved output plus the
// last (K-1) input columns as present_state, so streaming decode can continue.
//   Inputs : input[B,C,T], weight[C,1,K], (optional) bias[C], (optional) past_state[B,C,K-1]
//   Outputs: output[B,C,T], present_state[B,C,K-1]
