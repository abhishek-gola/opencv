                           ...
                           const float* bias__)
{
    ...
    // Fast path eligibility. For ngroups == 1 (and the typical multi-group case
    // where Kg % C0 == 0), every k0 in a single block shares the same group,
    // kblk, and k0l=k0 mapping -- which means one block's currK0 accumulators
    // can ride together in a single SIMD vector. Padded lanes in the last
    // block are safe because repackDeconvWeights() zero-fills the buffer.
    const bool simd_ok =
    ...
    parallel_for_(Range(0, NK1), [&](const Range& range) {
        for (int nk1 = range.start; nk1 < range.end; nk1++) {
            ...
            for (int opos_flat = 0; opos_flat < ospatial; opos_flat++) {
                ...
                // Scalar fallback (general path: groups crossing block boundaries,
                // or no SIMD).
                for (int ks = 0; ks < ksize; ks++) {
                    bool valid = true;
                    int ipos_flat = 0;
