...
// 2D 1x1 conv with stride=1, pad=0. Processes 2 adjacent Kblks (16 output channels)
// per task using a 6×16 AVX2 register tile: each input broadcast drives 2 FMAs
// (one per Kblk), halving broadcast pressure vs the universal-intrinsic 10×8 path.
// Pre-conditions (enforced by dispatcher): Kblk even, K%K0==0, ngroups==1, no residual init
// of unused lanes needed (k_count==K0 for both blocks).
                                 ...
                                 const float* scale__, const float* bias__)
{
    ...
    parallel_for_(Range(0, total_tasks), [&](const Range& range) {
        ...
        for (int t = range.start; t < range.end; t++) {
            ...
            // Scalar tail for positions [p, p1) — same math, one position at a time.
            for (; p < p1; p++)
            {
...
#if CV_SIMD256 && defined(__AVX2__)
// 2D 3x3 conv with stride=1, dilation=1, processing 2 adjacent Kblks (16 output channels) per
// task with a 6×16 AVX2 register tile. Same broadcast-amortization idea as conv32fC8_1x1_kpair.
// Dispatcher guarantees: ksize=9, strides=1, dilations=1, Kblk even, K%K0==0, ngroups==1.
                                  ...
                                  const float* scale__, const float* bias__)
{
    ...
    parallel_for_(Range(0, total_tasks), [&](const Range& range) {
        ...
        for (int t = range.start; t < range.end; t++) {
            ...
            // Scalar tail — handle each remaining position with 3x3 bounds checks.
            for (; p < p1; p++)
            {
                int yj = p / W;
                int xj = p - yj * W;
