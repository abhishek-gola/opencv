...
#ifdef HAVE_MLAS
    // MLAS handles contiguous row-major A/B with a single leading dim. The
    // post-swap (lda0,lda1) carry the strides of op(A): one of them is 1
    // (the inner dim) and the other is the physical storage row stride of
    // A. If neither is 1 we have a strided view that MLAS can't take.
...
#ifdef HAVE_MLAS
    // Derive the MLAS trans flag from the inner stride: lda1==1 means op(A)
    // matches the storage row-major layout (no trans); lda0==1 means we're
    // reading the storage transposed. Anything else is a strided view that
    // MLAS can't take.
    {
