 ...
 public:
    ...
    void forward(InputArrayOfArrays inputs_arr, OutputArrayOfArrays outputs_arr, OutputArrayOfArrays internals_arr) CV_OVERRIDE {
        ...
        if (blobs.empty()) {
            ...
            // MLAS's batched non-packed SGEMM beats fast_gemm's hand-rolled kernel on the
            // dynamic attention matmuls (Q*K^T, scores*V) in ViT-family models, where
            // one of M/N/K is small (head_dim, typically 64). MLAS has tuned skinny-K
            // paths that fast_gemm's generic blocking misses.
            bool done = false;
